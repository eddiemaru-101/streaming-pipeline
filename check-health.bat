...
echo [OK] 실행 중인 컨테이너: %healthy%개

if %healthy% lss 6 (
    echo [WARNING] 일부 컨테이너가 실행되지 않았습니다.
)
...
if %errorlevel% equ 0 (
    echo [OK] Flink Job 실행 중
) else (
    echo [ERROR] Flink Job 실행되지 않음
)
...
if %errorlevel% equ 0 (
    echo [OK] 실시간 프리미엄 계산 진행 중
) else (
    echo [WARNING] 아직 데이터 처리가 시작되지 않았거나 문제가 있습니다.
    echo    몇 분 더 기다려보세요.
)
